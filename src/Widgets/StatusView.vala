...
public class Vigil.Widgets.StatusView : Gtk.Box {
    ...
    construct {
        ...
        info_grid.add_css_class (Granite.STYLE_CLASS_CARD);

        // Status row
        var status_title = new Gtk.Label ("Status") {
            halign = Gtk.Align.START
        };
        ...
        info_grid.attach (status_title, 0, 0);
        info_grid.attach (status_label, 1, 0);

        // Backend row
        var backend_title = new Gtk.Label ("Screenshot method") {
            halign = Gtk.Align.START
        };
        backend_title.add_css_class ("dim-label");
        backend_label = new Gtk.Label ("Detecting\u2026") {
            halign = Gtk.Align.END,
            hexpand = true
        };
        info_grid.attach (backend_title, 0, 1);
        info_grid.attach (backend_label, 1, 1);

        // Next capture row
        var next_title = new Gtk.Label ("Next capture") {
            halign = Gtk.Align.START
        };
        next_title.add_css_class ("dim-label");
        next_capture_label = new Gtk.Label ("\u2014") {
            halign = Gtk.Align.END,
            hexpand = true
        };
        info_grid.attach (next_title, 0, 2);
        info_grid.attach (next_capture_label, 1, 2);

        // Last capture row
        var last_title = new Gtk.Label ("Last capture") {
            halign = Gtk.Align.START
        };
        last_title.add_css_class ("dim-label");
        last_capture_label = new Gtk.Label ("\u2014") {
            halign = Gtk.Align.END,
            hexpand = true
        };
        info_grid.attach (last_title, 0, 3);
        info_grid.attach (last_capture_label, 1, 3);

        // Pending uploads row
        var pending_title = new Gtk.Label ("Pending uploads") {
            halign = Gtk.Align.START
        };
        pending_title.add_css_class ("dim-label");
        pending_uploads_label = new Gtk.Label ("0") {
            halign = Gtk.Align.END,
            hexpand = true
        };
        info_grid.attach (pending_title, 0, 4);
        info_grid.attach (pending_uploads_label, 1, 4);
